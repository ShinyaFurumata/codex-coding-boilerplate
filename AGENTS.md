...
機能・UI 開発タスクは原則として TDD（レッド → グリーン → リファクタ）で進めてください。実装前に失敗するテストを作成し、`pnpm validate` を用いた検証結果を作業報告で共有してください。ExecPlan には、テスト観点と追加予定のケースを必ず盛り込んでください。

既存のテスト基盤に不足がある場合は、実装に進む前にテストセットアップの改善から着手してください。堅牢なテスト環境は、品質の高いコードベースの礎となります。

また、ExecPlan を用いた大規模な変更後だけでなく、軽微な修正後でも、作業を終えるたびに `pnpm validate` を実行し、ログを報告してください。

- `pnpm pre-commit`: Git コミット前に変更されたファイルのみをチェック（lint-staged 経由で自動実行）
- `pnpm validate`: 全体の品質チェック（typecheck、lint、format:check、test:unit を順次実行）

いずれかが失敗した場合は原因を特定してから次へ進みます。

# ドキュメント体系

## ライブラリ・API のドキュメント参照

コード生成、セットアップや構成手順、ExecPlan の作成、あるいはライブラリ/API のドキュメントが必要な場合は、常にコンテキスト 7 を使用してください。つまり、私が明示的に頼まなくても、Context7 MCP ツールを自動的に使用してライブラリ ID を解決し、ライブラリのドキュメントを取得する必要があるということです。
...
- **next-devtools**：Next.js の関する実装をコードベースから直接調査・補助する際に使用。`next dev` 実行中のアプリに接続し、ルーティングやサーバーアクションの解析を行う場合に利用してください。
...
- ブラウザ操作、スクリーンショット撮影用
  ...
  - **chrome-devtools**：ブラウザ上で再現するバグや、パフォーマンス・ネットワーク・コンソールに関する問題を調査する際に利用してください。こちらでもスクリーンショットをとることはできるので、すでにこちらを立ち上げている場合は、Playwright を新規で立ち上げる必要はありません。
  - pnpn dev や next dev コマンドを実行する前に、すでに開発サーバーが立ち上がっていないか確認してください。
  - Next.js 16 では `pnpm exec next dev --hostname ... --port ...` のように直接 `next dev` を呼び出してください
  - 取得した成果物(スクリーンショット・ログ)は `.agents/tmp` に保存し、完了報告で保存先パスを必ず共有してください
  - 起動ログと PID を `.agents/tmp` に記録し、終了時にプロセスと 利用したブラウザを確実に停止してください
